...
class Classifier:  # Renamed from TopicExtractor
    ...
    def _parse_summaries(self, content: str) -> List[Dict]:
        ...
        for i, current_match in enumerate(matches):
            try:
                ...
                posts.append(
                    {"index": index, "title": title, "summary": summary_content, "url": url}
                )
                successful_parses += 1
    ...
    def _make_classification_api_call_with_retry(
        self, prompt: str, title: str, categories: List[str], max_retries: int = 10
    ) -> str:
        """
        Make classification API call with robust retry mechanism.

        Args:
            prompt: The prompt to send to the API
            title: Title for logging purposes
            categories: Valid categories for classification
            max_retries: Maximum number of retry attempts

        Returns:
            Classification category, or "分类失败" if all retries failed
        """
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    ...
                    logger.info(
                        f"分类API重试第 {attempt + 1}/{max_retries} 次，针对: '{title[:30]}...'，等待 {delay:.1f} 秒..."
                    )
                    time.sleep(delay)
                ...
                response = requests.post(
                    f"{self.base_url}/chat/completions", headers=self.headers, json=data, timeout=30
                )
                ...
                if "choices" in response_data and len(response_data["choices"]) > 0:
                    ...
                    if category in categories:
                        if attempt > 0:
                            logger.info(
                                f"✅ 分类API调用成功 (第{attempt + 1}次尝试): '{title[:30]}...' -> {category}"
                            )
                        else:
                            logger.debug(f"分类成功: '{title[:30]}...' -> {category}")
                        return category
                    else:
                        logger.warning(f"API返回的分类 '{category}' 不在预定义列表中")
                        if attempt < max_retries - 1:
                            logger.warning("将重试以获取有效分类...")
                            continue
                        else:
                            logger.warning("所有重试完成，标记为'其他'")
                            return "其他"
                else:
                    logger.warning(f"API未返回有效分类响应")
                    continue

            except requests.exceptions.Timeout:
                logger.warning(f"分类API调用超时 (尝试 {attempt + 1}/{max_retries}) 针对 '{title[:30]}...'")
                if attempt == max_retries - 1:
                    logger.error(f"已达到最大重试次数，分类API调用超时")
                continue

            except requests.exceptions.ConnectionError:
                logger.warning(f"分类网络连接错误 (尝试 {attempt + 1}/{max_retries}) 针对 '{title[:30]}...'")
                if attempt == max_retries - 1:
                    logger.error(f"已达到最大重试次数，分类网络连接失败")
                continue

            except requests.exceptions.HTTPError as http_err:
                status_code = http_err.response.status_code if http_err.response else "unknown"
                logger.error(
                    f"分类HTTP错误 {status_code} (尝试 {attempt + 1}/{max_retries}) 针对 '{title[:30]}...': {http_err}"
                )

                # Don't retry on authentication errors (4xx)
                if http_err.response and 400 <= http_err.response.status_code < 500:
                    logger.error(f"分类认证或客户端错误，停止重试")
                    break

                if attempt == max_retries - 1:
                    logger.error(f"已达到最大重试次数，分类HTTP错误持续")
                continue

            except Exception as e:
                logger.error(
                    f"分类API调用出现未知错误 (尝试 {attempt + 1}/{max_retries}) 针对 '{title[:30]}...': {e}"
                )
                if attempt == max_retries - 1:
                    logger.error(f"已达到最大重试次数，分类未知错误持续")
                continue

        logger.error(f"分类达到最大重试次数 {max_retries}，针对 '{title[:30]}...'")
        return "分类失败"
    ...
    def _classify_summary_with_api(self, title: str, summary: str) -> str:
        ...
        # 使用带重试机制的API调用
        return self._make_classification_api_call_with_retry(
            prompt, title, categories, max_retries=10
        )
    ...
    def _generate_category_hotspot_summaries(self) -> Dict[str, str]:
        ...
        category_counts = Counter(
            ...
            if post["category"] not in ["分类失败", "内容为空"]
        )
        ...
        for category in top_categories:
            summary_text = self._generate_category_summary_with_api(
                category, posts_by_category[category]
            )
    ...
    def _extract_top_concepts_with_api(self, all_summaries_text: str, top_n: int = 3) -> List[str]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/chat/completions", headers=self.headers, json=data, timeout=60
            )
            response.raise_for_status()
            response_data = response.json()
    ...
    def _generate_concept_hotspot_summaries(self, top_n: int = 3) -> Dict[str, str]:
        ...
        all_summaries_text = "\n\n".join(
            [p.get("summary", "") for p in self.classified_summaries if p.get("summary")]
        )
        if not all_summaries_text:
...
def run(input_file: Optional[str] = None, output_file: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if (
            classification_successful or classifier.classified_summaries
        ):  # Proceed if we have any summaries
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=str, help="指定输出的分类结果JSON文件路径 (例如：data/my_classified.json)。如果未指定，则使用默认路径"
    )
